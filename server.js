...
if (!OPENWEATHER_API_KEY) {
  console.error(
    '警告: OPENWEATHER_API_KEYが設定されていません。天気機能が動作しません。'
  );
}

if (!UNSPLASH_ACCESS_KEY) {
  console.error(
    '警告: UNSPLASH_ACCESS_KEYが設定されていません。背景画像機能が動作しません。'
  );
}
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        throw new Error(
          `API認証エラー (401): APIキーが無効です。OpenWeatherMapで有効なAPIキーを取得してください。`
        );
      } else if (response.status === 429) {
        throw new Error(
          `API制限エラー (429): リクエスト制限に達しました。しばらく待ってから再試行してください。`
        );
      } else {
    ...
    console.log(
      `Weather data for ${formattedWeather.location}: ${formattedWeather.temperature}°C`
    );
...
app.get('/api/news', async (req, res) => {
  ...
  try {
    ...
    console.log(
      `Successfully parsed feed: ${feed.title}, items: ${feed.items.length}`
    );
