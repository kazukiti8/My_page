...
const modulesToCheck = [
    ...
    { file: 'js/news.js', patterns: ['window.errorHandler', 'handleError', 'failedFeeds'] },
...
if (fs.existsSync('js/error-handler.js')) {
    ...
    const features = [
        ...
        { name: 'Promiseエラーハンドラー', pattern: 'addEventListener.*unhandledrejection' },
    ...
    features.forEach(feature => {
        if (content.includes(feature.pattern) || content.includes(feature.name.replace(/[^a-zA-Z]/g, ''))) {
...
if (fs.existsSync('js/weather.js')) {
    ...
    if (weatherContent.includes('retryCount') && weatherContent.includes('maxRetries')) {
...
if (fs.existsSync('js/news.js')) {
    ...
    if (newsContent.includes('failedFeeds') && newsContent.includes('Promise.allSettled')) {
...
// 推奨事項の表示
console.log('\n📋 推奨事項:');
console.log('1. ブラウザでアプリケーションを開いてエラーハンドリングをテストしてください');
